...
export class CompanyAdminService {
  constructor(
    ...
    private companyAdminRepo: Repository<CompanyAdmin>,
  ...
  async create({ data }: { data: CreateCompanyAdminDTO }) {
    await this.dataSource.transaction(async (manager) => {
      ...
      companyAdmin.company = await this.companyService.checkCompany(
        data.companyId,
      );
  ...
  async getAll({
    ...
    const query = this.companyAdminRepo
      .createQueryBuilder('company_admin')
      .select([
        'company_admin.id',
        'company_admin.firstName',
        'company_admin.middleName',
        'company_admin.lastName',
      ])
    ...
    if (search) {
      query
        .where('companyAdmin.firstName LIKE :search', { search: `%${search}%` })
        .orWhere('companyAdmin.middleName LIKE :search', {
          search: `%${search}%`,
        })
        .orWhere('companyAdmin.lastName LIKE :search', {
          search: `%${search}%`,
        });
    }
  ...
  async getById(id: string): Promise<CompanyAdmin> {
    const data = await this.companyAdminRepo
      .createQueryBuilder('companyAdmin')
      .select([
        'companyAdmin.id',
        'companyAdmin.first_name',
        'companyAdmin.middle_name',
    ...
    if (!data)
      throw new NotFoundException(`Company Admin with id ${id} not found`);
