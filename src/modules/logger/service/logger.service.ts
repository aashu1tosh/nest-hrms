...
export class CustomLoggerService implements LoggerService {
    ...
    constructor(private configService: ConfigService) {
        ...
        const isProd = this.configService.get<Environment>('NODE_ENV') === Environment.PRODUCTION;
            ...
            transports.push(new winston.transports.Console({
                format: winston.format.combine(
                    ...
                    winston.format.printf(({ timestamp, level, message, context, ms, ...meta }) => {
                        ...
                        const ctxName = JSON.stringify(context ?? 'HRMS APPLICATION', null, 2);
                        ...
                        const extras = Object.keys(meta).length ? chalk.gray(JSON.stringify(meta)) : '';
                        const duration = typeof ms === 'string' ? chalk.magenta(`+${ms}`) : '';
    ...
    log(message: any, context?: string, meta?: Record<string, any>) {
        return this.logger.info(message, { context: context || this.context, ...meta });
    }

    error(message: any, trace?: string, context?: string, meta?: Record<string, any>) {
        return this.logger.error(message, { trace, context: context || this.context, ...meta });
    }

    warn(message: any, context?: string, meta?: Record<string, any>) {
        return this.logger.warn(message, { context: context || this.context, ...meta });
    }

    debug(message: any, context?: string, meta?: Record<string, any>) {
        return this.logger.debug(message, { context: context || this.context, ...meta });
    }

    verbose(message: any, context?: string, meta?: Record<string, any>) {
        return this.logger.verbose(message, { context: context || this.context, ...meta });
    }
}
